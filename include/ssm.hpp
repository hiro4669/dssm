...
//** DSSMのための関数を追加 ******************************************* ----->
	...
	/**
	 * ブロードキャストで受け取った情報をssm-coordinatorに送信する
	 */
	void addInfo(const char* data, uint32_t data_len, uint16_t port) {		
		add_info(data, data_len, port);
	}

	/**
	 * @brief ブロードキャストで受け取った情報をssm-coordinatorから取得する
	 * 
	 */
	void getInfo() {
		int size = get_info_size();
		if (size == -1) return; // 失敗
		char* buffer = (char*) malloc(size);
		memset(buffer, 0, size);
		if (buffer == NULL) {
			fprintf(stderr, "cannot create buffer in getInfo\n");
			fprintf(stderr, "errno : %d\n", errno);
			return;
		}

		int ret = get_info(buffer, size);
		if (!ret) {
			fprintf(stderr, "cannot get information in getInfo\n");
		}
