...
/* MessageQueue Buffer Size */
#define MSQ_BUF_SIZE 30
...
//** DSSMのための定義を追加 ******************************************* ----->
	MC_OFFSET,                                  // オフセットの設定
	MC_TCPCONNECTION,                           // バルク通信用経路の確立
	MC_UDPCONNECTION,						    // UDPバルク通信用経路の確立
	MC_ADDINFO,                                 // ブロードキャストで取得した情報を書き込む 
	MC_GETINFOSIZE,                             // ブロードキャストで取得した情報の総量を取得する
	MC_GETINFO,                                 // ブロードキャストで取得した情報を取得する
	MC_FAIL = 30,
// <-------- DSSMのための定義を追加
...
extern "C"
{
	...
	typedef struct
	{
		...
		int sem_lock_key;                       ///< for python
		int sem_cond_key;                       ///< for python
		pthread_mutex_t mutex;					///< 同期用mutex lock
		pthread_cond_t cond;					///< 同期用pthread condition
	} ssm_header;
	//} __attribute__((__packed__))ssm_header;
