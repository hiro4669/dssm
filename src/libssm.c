...
/* ssm-coordinatorに情報を書き込む */
int add_info(const char* data, uint32_t data_len, uint16_t port) {
	ssm_msg msg;
	if (data_len == 0 || data_len >= SSM_SNAME_MAX) {
		fprintf(stderr, "SSM ERROR :  data_len exceeds limit %d > %d\n", data_len, SSM_SNAME_MAX);
	}
	strncpy(msg.name, data, data_len); // copy data
	msg.ssize = data_len; // set data length
	msg.hsize = port;     // set port number
	/* communicate */
	if (!communicate_msg(MC_ADDINFO, &msg)) {
		return 0;
	}
	return 1;
}

/* ssm-coordinatorからブロードキャストで絵た情報の総量を取得する */
int get_info_size() {
	ssm_msg msg;
	if (!communicate_msg(MC_GETINFOSIZE, &msg)) {
		return 0;
	}
	return msg.ssize;	
}

int get_info(char* buffer, uint32_t buf_len) {
	ssm_msg msg;
	send_msg(MC_GETINFO, &msg ); // send to ssm-coordinator
	...
	int idx = 0;
	while (1) {
		len = msgrcv( msq_id, &hinfo, MSQ_BUF_SIZE, my_pid, 0);
		memcpy(&buffer[idx], hinfo.buffer, len);
		idx += len;		
