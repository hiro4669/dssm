...
int msq_loop( void )
{
	...
	while( 1 )
	{
		...
		case MC_ADDINFO: {// for add host information
			if( verbosity_mode >= 2 )
				printf( "message: add information\n" );

			uint32_t data_len = msg.ssize;
			uint16_t port = msg.hsize;

			std::string ip_addr_str = std::string(msg.name, data_len);
			std::cout << "ip:port   " << ip_addr_str<< " : " << port << std::endl;			

			msg.msg_type = msg.res_type;	/* 返信 */
			msg.cmd_type = MC_RES;

			Neighbor nb(ip_addr_str, port); // dummy
			if (nbManager.find(nb)) {
			...
			std::cout << "neighbor size = " << nbManager.count() << std::endl;
		...
		case MC_GETINFOSIZE: {
			...
			msg.ssize = nbManager.count() * IPINFO_SIZE + 2;
		...
		case MC_GETINFO: {
			...
			hinfo.msg_type = msg.res_type;

			// get information from neighbor manager			
			int total_len = nbManager.count() * IPINFO_SIZE + 2;
			char* buffer = (char*)malloc(total_len);
			nbManager.serialize(buffer);
						
			int write_len = 0;			
			while (true) {
				int remaining = total_len - write_len;
				int len = 0;
				len = remaining < MSQ_BUF_SIZE ? remaining : MSQ_BUF_SIZE;				
				memcpy(hinfo.buffer, &buffer[write_len], len);
				if( ( msgsnd( msq_id, &hinfo, len, 0 ) ) < 0 )
					return 0;
				write_len += len;
